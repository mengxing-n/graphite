function [ Ekfit, EDCfit, Param] = exp_Lorentzian_fit( Ek, EDC, Weight, Var_num, Prob_num, Var_in, Prob_in )
...
f = fittype('exp_lorentz_fun5( x, A1, A2, A3, A4, A5, B1, B2, B3, B4, B5, C1, D1, a, b, c, d)','indep','x',...
    'coeff', Coeff, 'problem', Prob,'options', Opt);

[c2,~] = fit((Ek)',EDC,f, 'Weight',Weight', 'problem',Prob_val);
Ekfit=linspace(min(Ek),max(Ek),1064);
EDCfit=feval(c2,Ekfit);
C=coeffvalues(c2);
CI=confint(c2);
Param=zeros(3, length(C));
Param(1,:)=C;
Param(2:3,:)=CI;
